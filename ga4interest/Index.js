...
app.get('/analyze', async (req, res) => {
  try {
    ...
    res.json({
      ...
      top_root: finalData.root.slice(0, 10).map(i => ({
        name: i.hierarchy, score: i.interest_score, trend: i.trend
      })),
      top_sub: finalData.sub.slice(0, 10).map(i => ({
        name: i.hierarchy, score: i.interest_score, trend: i.trend
      })),
      top_products: finalData.product.slice(0, 10).map(i => ({
        name: i.slug || i.productId, score: i.interest_score, trend: i.trend
      })),
...
app.get('/', (req, res) => {
  res.json({
    status: 'running',
    service: 'Kapruka Category Interest Analyzer',
    endpoints: {
      '/analyze': 'Run full analysis (GET)',
      '/download/:file': 'Download generated Excel'
    }
  });
});
